...
# 2. Définir les infos de l'API (Logements Existants - Dép. 31)
...
print("Début de la récupération des données (Logements Existants - Dép. 31)...")
...
while (TRUE) {
  ...
  if (response$status_code == 200) {
    ...
    if (!is.null(rows_in_this_page) && rows_in_this_page > 0) {
      ...
      print(paste("... ", total_rows_fetched, "lignes (existants) récupérées..."))
      Sys.sleep(0.1) 
    } else {
      print("Fin de la récupération (existants).")
      break 
    }
  } else {
...
if (length(all_data_list) > 0) {
  df_existants_31 <- do.call(rbind, all_data_list)
  saveRDS(df_existants_31, "donnees_dpe_existants_31.rds")
  print("Terminé ! Les données EXISTANTS sont dans 'donnees_dpe_existants_31.rds'")
} else {
  print("Aucune donnée n'a été récupérée pour les logements existants.")
}