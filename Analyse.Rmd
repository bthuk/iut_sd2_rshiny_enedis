...
# Paquets requis
pkgs <- c("shiny","tidyverse","DT","leaflet","sf","bslib","ggplot2","plotly","corrplot","broom","janitor","glue")
installed <- rownames(installed.packages())
for(p in pkgs) if(!p %in% installed) install.packages(p, dependencies = TRUE)

# Charger les libraries
library(shiny); library(tidyverse); library(DT); library(leaflet); library(sf)
library(bslib); library(ggplot2); library(plotly); library(corrplot); library(broom)
library(janitor); library(glue)

# Thèmes disponibles : clair / sombre
